...
export default function DictionaryComponent({ dictionary }: Props) {
  ...
  const confirmDelete = useCallback(() => {
    // 检查是自定义词典还是自定义文章
    if (dictionary.id.startsWith('custom-article-')) {
      // 删除自定义文章
      const customArticles = JSON.parse(localStorage.getItem('custom-articles') || '[]')
      const updatedArticles = customArticles.filter((article: any) => article.id !== dictionary.id)
      localStorage.setItem('custom-articles', JSON.stringify(updatedArticles))
    } else {
      // 删除自定义词典
  ...
  return (
    <>
            ...
            <div className="relative ml-1 mt-2 flex h-full w-full flex-col items-start justify-start">
              ...
              {isCustomDict && (
                <div className="absolute bottom-0 right-0 flex space-x-1">
                  {/* 只有自定义词典才显示编辑按钮，自定义文章不支持编辑 */}
                  {!dictionary.id.startsWith('custom-article-') && (
                    <button
                      onClick={handleEdit}
                  ...
                  <button
                    ...
                    title={dictionary.id.startsWith('custom-article-') ? '删除文章' : '删除词典'}
                  >
      ...
      {/* 编辑模态框 */}
      {showEditModal && <CustomDictionaryEditModal dictionary={dictionary} onClose={() => setShowEditModal(false)} />}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md transform rounded-lg bg-white p-6 shadow-xl transition-all">
            ...
            <p className="mt-2 text-sm text-gray-500">确定要删除词典 "{dictionary.name}" 吗？此操作无法撤销。</p>
